...
WebVTT.processCues = function(window, cues, overlay) {
  ...
  (function() {
    ...
    for (var i = 0; i < cues.length; i++) {
      ...
      const isPortrait = window.innerWidth < window.innerHeight ? true : false;
      let styleOptions;
      if (isPortrait && window.document.body.classList.contains('vjs-full-window')) {
        styleOptions = {
          font: "5vw/2.2em " + FONT_STYLE,
          fontWeight: "900"
        };
      } else {
        if (isPortrait && fontSize > 30) {
          fontSize = 30;
        }
        styleOptions = {
